...
Rat.prototype.type = "rat";


// Drain is initialized based on the character, but otherwise has a
// size and position
function Drain(pos, ch) {
  this.pos = pos;
  this.size = new Vector(1, 1);
  if (ch == "=") {
    // Horizontal drain
    ...
    // Drip drain. Repeat back to this pos.
    this.speed = new Vector(0, 3);
    this.repeatPos = pos;
  }
}
Drain.prototype.type = "drain";
...
Drain.prototype.act = function(step, level) {
...
Rat.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  if (obstacle=='wall') {
	...
	else if (obstacle=='drain')
		  {
			 this.pos=new Vector (5,5); 
		  }
	  else if(obstacle=='rat')
	  {
		  this.pos=new Vecotr (5,5);
  }
  else {
    this.pos = newPos;
  }
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "drain" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  }
	else if(type == "rat" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin") {
